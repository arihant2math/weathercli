...
fn get_forecast_sentence(data: Vec<Rc<dyn WeatherDataRS>>, raw_data: MeteoForecastJson, start: usize) -> String {
        ...
        if data[0].clone().get_conditions().into_iter()
            ...
            if rain_start.is_some() {
                return format!(
                    "It will rain in {} hours for {} hours", rain_start.unwrap(), rain_end.unwrap() - rain_start.unwrap())
            }
            if snow_start.is_some() {
                return format!("It will snow in {} hours for {} hours", snow_start.unwrap(), snow_end.unwrap() - snow_start.unwrap())
            }
        }
...
impl WeatherForecastRS for MeteoForecast {
    fn new(coordinates: Vec<String>, settings: Settings) -> Self {
        let data = meteo_get_combined_data_formatted(coordinates.clone(), settings.internal.metric_default.unwrap());
        ...
        for i in now + 1 .. data.weather.hourly.time.clone().len() {
            forecast.push(Rc::new(MeteoFuture::new(data.weather.clone(),
                                                   data.air_quality.clone(),
                                                   settings.internal.metric_default.unwrap(),
                                                   i)));
        }
