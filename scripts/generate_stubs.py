...
def arguments_stub(
    ...
    for match in re.findall(r"^ *:type *([a-z_]+): ([^\n]*) *$", doc, re.MULTILINE):
        if match[0] not in real_parameters:
            raise ValueError(
                f"The parameter {match[0]} of {'.'.join(element_path)} is defined in the documentation but not in the "
    ...
    for param in real_parameters.values():
        if param.name != "self" and param.name not in parsed_param_types:
            # raise ValueError(
            #     f"The parameter {param.name} of {'.'.join(element_path)} has no type definition in the function "
            #     f"documentation"
            # )
            continue
        else:
            ...
            if param.default != param.empty:
                ...
                if param.name not in optional_params:
                    raise ValueError(
                        f"Parameter {param.name} of {'.'.join(element_path)} is optional according to the type but not "
                        f"flagged as such in the doc"
                    )
            elif param.name in optional_params:
                raise ValueError(
                    f"Parameter {param.name} of {'.'.join(element_path)} is optional according to the documentation but "
...
def returns_stub(
    ...
    if len(m) == 0:
        ...
        if isinstance(builtin, tuple) and builtin[1] is not None:
            return builtin[1]
        # raise ValueError(
        #     f"The return type of {'.'.join(element_path)} has no type definition using :rtype: in the function "
        #     f"documentation"
        # )
        return None
    elif len(m) == 1:
